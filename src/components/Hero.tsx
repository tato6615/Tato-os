...
export default function Hero() {
  ...
  return (
    <section className="relative w-full overflow-hidden h-screen bg-black" style={{ height: '100dvh' }}>
      ...
      {/* 2. Reveal layer — cursor-following spotlight mask */}
      ...
      {menuOpen && (
        <div className="md:hidden fixed top-16 left-4 right-4 z-[100] bg-white/95 backdrop-blur-md rounded-2xl p-4 flex flex-col gap-1 shadow-xl">
          {NAV_ITEMS.map((item) => (
            <button
              key={item}
              className="text-left px-4 py-2 rounded-xl text-sm font-medium text-gray-800 hover:bg-gray-100"
            >
              {item}
            </button>
          ))}
          <button className="mt-2 bg-gray-900 text-white text-sm font-semibold px-6 py-2.5 rounded-full">
            Sign Up
          </button>
        </div>
      )}
