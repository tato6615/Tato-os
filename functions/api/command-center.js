...
export async function onRequestGet(context) {
  ...
  try {
    ...
    const behaviorRows =
      behaviorTypes.results || [];

    const attentionEvents =
      behaviorRows.filter(row =>
        [
          "view",
          "page_view",
          "product_view",
    ...
    if (attentionTotal > 0) {
      decisions.push({
        ...
        action:
          "สร้าง Content หรือ Offer จากสิ่งที่ผู้ใช้กำลังสนใจ",
        reason:
          `พบ Attention Signal ${attentionTotal} ครั้ง`,
        status: "READY"
      });
    }
    ...
    if ((latestMarket.results || []).length > 0) {
      decisions.push({
        ...
        action:
          "ตรวจสอบ Market Signal ล่าสุดและสร้างข้อเสนอที่ตอบ Demand",
        reason:
          `พบ Market Signals ${latestMarket.results.length} รายการ`,
        status: "READY"
      });
    }

    if (
      Number(customersCount?.total || 0) > 0
    ) {
      decisions.push({
        type: "CUSTOMER",
        priority: "MEDIUM",
        ...
        action:
          "แบ่งกลุ่มลูกค้าตามพฤติกรรมและระดับความสนใจ",
        reason:
          `มีลูกค้า ${customersCount.total} รายการ`,
        status: "READY"
      });
    }

    if (
      Number(aiInsightsCount?.total || 0) > 0
    ) {
      decisions.push({
        type: "AI_INSIGHT",
        priority: "HIGH",
        ...
        action:
          "นำ AI Insight ล่าสุดไปตรวจสอบและเปลี่ยนเป็น Action",
        reason:
          `มี AI Insights ${aiInsightsCount.total} รายการ`,
        status: "READY"
      });
    }
        ...
        switch (
          ...
          case "ATTENTION":
            return {
              action_type: "CONTENT",
              title:
                "สร้าง Content จาก Attention Signal",
              description:
                "นำสิ่งที่ผู้ใช้สนใจมาสร้าง Content หรือ Offer",
              priority:
                decision.priority,
              status: "READY",
              source: "ATTENTION"
          ...
          case "MARKET":
            return {
              ...
              title:
                "ตอบสนองต่อ Market Signal",
              description:
                "นำ Demand ที่พบมาสร้างข้อเสนอหรือ Content",
              priority:
                decision.priority,
              status: "READY",
              source: "MARKET"
          ...
          case "CUSTOMER":
            return {
              ...
              title:
                "แบ่งกลุ่มลูกค้า",
              description:
                "จัดกลุ่มลูกค้าตามพฤติกรรมเพื่อทำข้อเสนอเฉพาะกลุ่ม",
              priority:
                decision.priority,
              status: "READY",
              source: "CUSTOMER"
          ...
          case "AI_INSIGHT":
            return {
              ...
              title:
                "เปลี่ยน AI Insight เป็น Action",
              description:
                "ตรวจสอบ Insight และสร้าง Action ที่วัดผลได้",
              priority:
                decision.priority,
              status: "READY",
              source: "AI_INSIGHT"
